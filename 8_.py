...
# 2019-01-20            added custom_map_exploring, will try to move deeper to make dropoff
#                       will save halite to make dropoff if cost is just a little bit more than halite stored
#                       Updated spawn ship logic, set "collect turn = no. of players - 1"
#####################################################################################################################
...
def spawn_ship():
    ...
    collect_turn = len(game.players) - 1
    explore_distance = int(int((constants.MAX_TURNS - game.turn_number -1) / 2) / (1+collect_turn))
    _cost = (constants.SHIP_COST + save_halite_for_dropoff) if save_halite_for_dropoff else constants.SHIP_COST
...
def custom_map_exploring(ship, any_map, explore_distance=1):
    # Explore using density
    distance_map = distance_map_from_position(ship.position, safe=False, turns=9999)
    # any_map[ship.position.y, ship.position.x] += ship.halite_amount
    target = np.unravel_index(((distance_map <= explore_distance) * any_map).argmax(), any_map.shape)
    if any_map[ship.position.y, ship.position.x] >= any_map[target] * 0.95:
        return False
    move = get_optimize_naive_move(ship.position, Position(target[1],target[0]))
    if move != Direction.Still \
            and not check_enemy_ship_nearby(normalize_directional_offset(ship.position, move)) \
...
while True:
    ...
    # # return early and spawn more ships at game early state
    # if game.turn_number <= 50 and len(me.get_ships()) < 8:
    #     cust_constants.MAX_HALITE_RETURN = 650
    # else:
    ...
    for ship in me.get_ships():
        ...
        if _convert_dropoff_signal != 0:
            ship_status[ship.id] = "convert_dropoff"
            # me_halite_left -= get_convert_dropoff_cost(ship)
            make_dropoff = ship.id
            save_halite_for_dropoff = get_convert_dropoff_cost(ship)
            continue
        elif ship_status[ship.id] == "convert_dropoff":
            ship_status[ship.id] = "exploring"
